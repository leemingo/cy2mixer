...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
    ...
    dataset.cache_file_name = f"/data/mm{dataset.cache_file_name[1:]}"
    dataset.cache_file_folder = f"/data/mm{dataset.cache_file_folder[1:]}"
    # 转换数据，并划分数据集
    train_data, valid_data, test_data = dataset.get_data()
    data_feature = dataset.get_data_feature()
    # 加载执行器
    model_cache_file = '/data/mm/libcity/cache/{}/model_cache/{}_{}.m'.format(
